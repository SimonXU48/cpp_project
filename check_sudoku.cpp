...
bool check_sequence(const std::vector<int>& v)
{
    ...
    int index;
    int sequence_len = v.size();
    std::vector<int> num_list(sequence_len);
    ...
    for(int i = 0; i < sequence_len; i++)
    {
        ...
        index = mfind(v[i], num_list);

        if(index == -1)
            return false;

        num_list.erase(num_list.begin() + index);
...
void get_row(int r, const std::vector<int>& in, std::vector<int>& out){
    // write your code here
    int index;
    int row_num = std::sqrt(in.size());

    for(int i = 0; i < row_num; i++)
    {
        index = twod_to_oned(r, i, row_num); 
...
void get_col(int c, const std::vector<int>& in, std::vector<int>& out){
    // write your code here
    int index;
    int col_num = std::sqrt(in.size());

    for(int i = 0; i < col_num; i++)
    {
        index = twod_to_oned(i, c, col_num);
        out.push_back(in[index]);
    }
}
...
void get_subsq(int subs, const std::vector<int>& in, std::vector<int>& out){
    ...
    int subsq_num = std::sqrt(in.size());
    int subsq_num_rows = std::sqrt(subsq_num);
    int subsq_col_index = subs % subsq_num_rows;
    int subsq_row_index = subs / subsq_num_rows;

    for(int i = 0; i < subsq_num_rows; i++) // for loop for the rows
    {
        for(int j = 0; j < subsq_num_rows; j++) // for loop for the cols
        {
            row_index = subsq_row_index * subsq_num_rows + i; 
            col_index = subsq_row_index * subsq_num_rows + j;
            index = twod_to_oned(row_index, col_index, subsq_num);
